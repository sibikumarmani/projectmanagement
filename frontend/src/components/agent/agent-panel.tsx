...
export function AgentPanel({ compact = false }: AgentPanelProps) {
  ...
  return (
    <div className={`grid gap-4 ${compact ? "h-full" : "xl:grid-cols-[minmax(0,1fr)_360px]"}`}>
      <SectionCard
      ...
      >
        <div className={compact ? "flex h-full min-h-0 flex-col" : ""}>
          <div className={`space-y-4 ${compact ? "min-h-0 flex-1 overflow-y-auto pr-1" : ""}`}>
              ...
              <div
                ...
                className={`rounded-[24px] px-5 py-4 ${
                  message.role === "user"
                    ...
                    : "max-w-4xl border border-line surface-soft text-[color:var(--foreground)]"
                }`}
              >
            ...
            {isSending ? (
              <div className="surface-soft max-w-3xl rounded-[24px] border border-line px-5 py-4 text-sm text-[color:var(--foreground-muted)]">
          ...
          <div className={`${compact ? "mt-4 shrink-0" : "mt-6"} rounded-[28px] border border-line surface-glass p-4`}>
            <textarea
              className={`field w-full resize-none rounded-[22px] px-4 py-4 text-sm ${
      ...
      {!compact ? (
        <SectionCard title="What It Can Do" eyebrow="Guide">
          <div className="space-y-4 text-sm leading-7 text-[color:var(--foreground-muted)]">
            <div className="surface-glass rounded-[24px] px-5 py-4">
