...
export const metadata: Metadata = {
  title: "PMS Control Center",
  description: "Project Management and Cost Control System",
};

const themeBootstrapScript = `
(() => {
  const themeKey = "pms-theme-mode";
  const accentKey = "pms-accent-color";
  const defaultAccent = "#138a9e";
  ...
  const normalizeHexColor = (value) => {
    ...
    return fullMatch ? "#" + fullMatch[1].toLowerCase() : defaultAccent;
  };
  ...
  const mixHexColors = (base, mixWith, mixWeight) => {
  ...
  const buildThemeCssVariables = (mode, accentColor) => {
  ...
  const mode = localStorage.getItem(themeKey) === "dark" ? "dark" : "light";
  const accent = normalizeHexColor(localStorage.getItem(accentKey));
